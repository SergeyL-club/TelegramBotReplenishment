...
export async function use_toggle_admin_on(telegram_controller: TelegramController, user_manager: UserManager): Promise<void> {
  telegram_controller.use({ kid: "callback_query", math: (_, msg) => msg.data === "toggle_admin_on" }, async (ctx, _, binds) => {
    await ctx.answerCbQuery();
    await user_manager.toggle_admin_by_user_id(ctx.from!.id, true);
  ...
  await default_logger.info("Registration use_toggle_admin_on is finish");
}
