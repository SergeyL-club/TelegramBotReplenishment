...
export async function use_method_del(telegram_controller: TelegramController, deal_manager: DealManager): Promise<void> {
  telegram_controller.use({ kid: "callback_query", math: (_, msg) => msg.data === "del_method_name" }, async (ctx, cb, binds) => {
  ...
  telegram_controller.use({ kid: "reply", math: (_, __, binds) => binds[0] === "del_get_method_name" }, async (ctx, msg, binds) => {
    ...
    if (!(await deal_manager.verification_by_method_name(method_name))) {
      await ctx.reply(`Метод ${method_name} не существует в списках`);
      return;
    }
    await deal_manager.delete_method(method_name);
  ...
  await default_logger.info("Registration use_method_del is finish");
}
